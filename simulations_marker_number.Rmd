...
```{r parameters}
chromosomes <- 5
individuals <- 1000 # number of individual plants that would be planted
...
```{r}
marker.names <- unlist(lapply(map,names))
cross.small <- drop.markers(cross,sample(marker.names,2400,replace = FALSE))
cross.small
```
